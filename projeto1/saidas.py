...
FILENAME = 'saidas.txt'
FIELDS = ['id', 'cliente', 'itens', 'total', 'data']
...
def open_saidas_window(master=None):
    ensure_file()
    ...
    def refresh():
        ...
        for r in load_records():
            lb.insert('end', f"{r['id']} - {r['cliente']} - {r['total']} - {r['data']}")
    ...
    def on_add():
        ...
        if not new.get('data'):
            new['data'] = datetime.now().strftime('%Y-%m-%d %H:%M')
        records.append(new)
